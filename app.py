...
# Title
st.markdown('<h1 class="main-header">📊 Web-Based Interactive Data Mining System</h1>',
            unsafe_allow_html=True)
...
# Data Upload Page
    ...
    st.markdown('<h2 class="sub-header">📁 Data Upload</h2>',
                unsafe_allow_html=True)
    ...
    if st.button("📊 Load StudentsPerformance.csv"):
        try:
            ...
            st.success(
                f"✅ StudentsPerformance.csv loaded! ({df.shape[0]} rows, {df.shape[1]} columns)")
            ...
            st.error(
                "❌ File not found. Please ensure 'data/StudentsPerformance.csv' exists.")
    ...
    if st.session_state.dataset is not None:
        ...
        st.success(
            f"✅ Dataset is loaded: {df.shape[0]} rows × {df.shape[1]} columns")
        st.write(
            "Available for: Data Preprocessing, Classification, Clustering, etc.")

# Data Preprocessing Page
elif page == "🧹 Data Preprocessing":
    st.markdown('<h2 class="sub-header">🧹 Data Preprocessing</h2>',
                unsafe_allow_html=True)
    ...
    if missing_cols:
        ...
        fill_method = st.selectbox(
            ...
            ["Drop rows", "Mean fill (numeric)", "Median fill (numeric)",
             "Mode fill (categorical)", "Custom value"]
        )
        ...
        if st.button("Apply Missing Value Handling", key="missing_values"):
            if fill_method == "Drop rows":
                df = df.dropna()
                st.success(
                    f"✅ Dropped rows with missing values. Remaining: {len(df)} rows")
    ...
    if categorical_cols:
        ...
        encode_cols = st.multiselect("Select columns to encode:", categorical_cols,
        ...
        if st.button("Apply Feature Encoding", key="encoding"):
            if encoding_method == "Label Encoding":
                ...
                st.success(
                    f"✅ Applied Label Encoding to {len(encode_cols)} columns")
            else:
                df = pd.get_dummies(df, columns=encode_cols, drop_first=True)
                st.success(
                    f"✅ Applied One-Hot Encoding. New features: {len(df.columns)}")
    else:
    ...
    if numeric_cols:
        ...
        scale_cols = st.multiselect(
            "Select columns to scale:", numeric_cols, default=numeric_cols[:min(3, len(numeric_cols))])
    ...
    if st.checkbox("Create derived features"):
        ...
        if len(score_cols) >= 2:
            ...
            if 'total_score' not in df.columns:
                # FIX: Change selectbox to multiselect
                total_score_cols = st.multiselect(
                    "Select columns to sum for total score:",
                    score_cols,
                    default=score_cols[:min(3, len(score_cols))]
                )
                if st.button("Create Total Score"):
                    if total_score_cols:  # Check if any columns are selected
                        df['total_score'] = df[total_score_cols].sum(axis=1)
                        st.success(
                            f"✅ Created total_score from {len(total_score_cols)} columns")
                    else:
                        st.warning(
                            "Please select at least one column to create total score.")

            # Create pass/fail column
            if 'total_score' in df.columns and 'passed' not in df.columns:
                threshold = st.slider(
                    "Passing threshold (total score):", 0, 300, 180)
                if st.button("Create Pass/Fail Column"):
                    df['passed'] = (df['total_score'] >= threshold).astype(int)
                    st.success(
                        f"✅ Created 'passed' column (threshold: {threshold})")
...
# Classification Page
    ...
    st.markdown('<h2 class="sub-header">🌲 Classification Analysis (Decision Tree)</h2>',
                unsafe_allow_html=True)
    ...
    if 'total_score' not in df.columns and len(score_cols) >= 1:
        if st.button("Calculate Total Score"):
            ...
            st.success(
                f"✅ Created total_score from {len(score_cols)} score columns")

    # Create target variable options
    target_option = st.radio(
        "Choose target variable to predict:",
        ["Pass/Fail (Binary)", "Math Score Level", "Reading Score Level",
         "Writing Score Level", "Total Score Level"]
    )
    ...
    if target_option == "Pass/Fail (Binary)":
        if 'total_score' in df.columns:
            ...
            st.error(
                "❌ Need total_score column. Please create it first or select different target.")
            st.stop()
        ...
        if score_col:
            bins = st.slider(
                f"Number of bins for {score_type} score:", 3, 10, 4)
            df['target'] = pd.qcut(df[score_col], q=bins, labels=[
                                   f'Level_{i}' for i in range(bins)])
    ...
    available_features = [
        col for col in df.columns if col not in exclude_cols and df[col].nunique() > 1]
            ...
            try:
                ...
                sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                            xticklabels=np.unique(
                                y_test) if y.dtype != 'object' else le_target.classes_,
        ...
        if selected_features:
            ...
            if st.button("Predict"):
                ...
                if st.session_state.le_target is not None:
                    result = st.session_state.le_target.inverse_transform([prediction])[
                        0]
                else:
...
# Clustering Page
    ...
    st.markdown('<h2 class="sub-header">📊 Clustering Analysis (K-Means)</h2>',
                unsafe_allow_html=True)
    ...
    if not numeric_cols:
        st.error(
            "❌ No numeric features available for clustering. Please preprocess data first.")
        st.stop()
    ...
    if st.button("🔍 Perform K-Means Clustering", type="primary"):
        ...
        cluster_sizes['Percentage'] = (
            cluster_sizes['Count'] / len(df_clustered) * 100).round(1)
...
# Association Rules Page
    ...
    st.markdown('<h2 class="sub-header">🔗 Association Rule Mining (Apriori)</h2>',
                unsafe_allow_html=True)
    ...
    numerical_level_features = [
        col for col in df.columns if 'level' in col.lower()]
    ...
    if not all_features:
        ...
        st.info(
            "Please ensure your dataset has categorical columns or create level columns from scores.")
        st.stop()
        ...
        with st.spinner("Mining association rules..."):
            try:
                ...
                if len(frequent_itemsets) == 0:
                    st.warning(
                        "⚠️ No frequent itemsets found. Try lowering the minimum support.")
                else:
                    ...
                    if len(rules) == 0:
                        st.warning(
                            "⚠️ No association rules found with current parameters. Try adjusting confidence or lift.")
                    else:
                        ...
                        with col1:
                            ...
                            for idx, rule in top_conf.iterrows():
                                ...
                                st.write(
                                    f"**{idx+1}.** {antecedents[0] if antecedents else ''} → {consequents[0] if consequents else ''}")
                                st.write(
                        ...
                        with col2:
                            ...
                            for idx, rule in top_lift.iterrows():
                                ...
                                st.write(
                                    f"**{idx+1}.** {antecedents[0] if antecedents else ''} → {consequents[0] if consequents else ''}")
                                st.write(
                                    f"   Lift: {rule['lift']:.2f}, Confidence: {rule['confidence']:.2%}")
                        ...
                        if len(rules) > 0:
                            ...
                            # Lift distribution
                            axes[1].hist(rules['lift'], bins=20,
                                         edgecolor='black', alpha=0.7)
                            axes[1].axvline(x=1.0, color='red',
                                            linestyle='--', label='Independent')
                ...
                st.info(
                    "💡 Try selecting different features or adjusting parameters.")
...
# Visualization Page
    ...
    st.markdown('<h2 class="sub-header">📈 Visual Analytics Dashboard</h2>',
                unsafe_allow_html=True)
    ...
    viz_type = st.selectbox(
        ...
        ["Distribution Analysis", "Correlation Analysis",
            "Score Analysis", "Group Comparison", "Clustering Results"]
    )
    ...
    if viz_type == "Distribution Analysis":
        ...
        with col1:
            ...
            if numeric_cols:
                selected_num_col = st.selectbox(
                    "Select numerical feature:", numeric_cols)
        ...
        if score_cols:
            selected_scores = st.multiselect(
                "Select score columns:", score_cols, default=score_cols[:min(3, len(score_cols))])
        ...
        if 'clusters' in st.session_state and st.session_state.clusters is not None:
            ...
            numeric_cols = df_clustered.select_dtypes(
                include=[np.number]).columns.tolist()
            ...
            if len(numeric_cols) >= 2:
                ...
                y_col = st.selectbox(
                    "Y-axis feature:", [col for col in numeric_cols if col != x_col])
            ...
            st.info(
                "ℹ️ No clustering results found. Please perform clustering analysis first.")

# Performance Evaluation Page
elif page == "📋 Performance Evaluation":
    st.markdown('<h2 class="sub-header">📋 Model Performance Evaluation</h2>',
                unsafe_allow_html=True)

    if 'trained_model' not in st.session_state or st.session_state.trained_model is None:
        st.warning(
            "⚠️ Please train a classification model first in the 'Classification' page.")
        st.stop()
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=np.unique(y_test) if len(
                    np.unique(y_test)) < 10 else range(len(np.unique(y_test))),
                yticklabels=np.unique(y_test) if len(
                    np.unique(y_test)) < 10 else range(len(np.unique(y_test))),
                ax=ax)
    plt.title('Confusion Matrix')
    plt.ylabel('Actual')
